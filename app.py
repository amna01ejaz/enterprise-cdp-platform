...
# FIXED: Strict schema format matching modern streamlit-authenticator models
credentials = {
    "usernames": {
        "amnaeiaz": {
            "name": "Amna Ejaz",
            "password": hashed_passwords[0],
            "email": "amnaeiaz@enterprise.com"
        },
        "recruiter": {
...
# Instantiating the authentication state module
authenticator = stauth.Authenticate(
    credentials=credentials, 
    cookie_name="cdp_cookie", 
    cookie_key="auth_key", 
    cookie_expiry_days=1
)

# Render login window interface frame component
name, authentication_status, username = authenticator.login(location='main')

# Evaluating Authorization State Gates
if authentication_status == False:
    st.error('Access Denied: The password or username combination is incorrect.')
elif authentication_status == None:
    st.warning('🔒 Access Restricted: Please enter administrative credentials to mount the Data Platform.')
elif authentication_status:
    
    # Render operational sidebar controls once logged in
    st.sidebar.markdown(f"👤 **Session User:** {name}")
    authenticator.logout('Secure Sign Out', 'sidebar')
    st.sidebar.markdown("---")
